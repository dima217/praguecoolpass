...
const ReviewForm = ({ active, setActive, onSuccess, onReset, popupTranslations }) => {
  ...
  return (
    <div
      className={`fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center transition-opacity z-[202] ${
    ...
    >
      <div
        ref={containerRef}
        className="bg-white md:rounded-xl md:px-[50px] px-2 md:pb-[25px] max-w-[855px] w-full h-[90vh] md:h-fit relative overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="absolute right-4 md:top-5 top-10">
          <button className="text-black" onClick={() => setActive(false)}>
            <AiOutlineClose size={24} />
          </button>
        </div>

        <div className="my-[23px] mt-10">
          <h2 className="text-[22px] font-bold pb-[15px]">
            {popupTranslations.WRITE_REVIEW.toUpperCase()}
          </h2>
          <p className="text-sm">{popupTranslations.REVIEWS_review_form_text}</p>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="mb-[5px]">
          <div className="grid md:grid-cols-2 grid-rows-4 md:grid-rows-2 gap-4">
            <CustomInput
              error={errors.name}
              register={register}
          ...
          <div className="mt-4">
            <textarea
              ...
              {...register('message')}
              className={`block w-full border-[0.8px] ${
                errors.message ? 'border-red-500' : 'border-[#dbdbdb]'
              } shadow-sm sm:text-sm mt-[10px] p-[15px] focus:outline-none focus:border-[#585858] text-sm resize-none h-[150px] md:h-[200px]`}
            />
          </div>
          ...
          <div className="text-right mt-6">
            <button
              type="submit"
              disabled={!isValid}
              className={`h-[48px] md:w-[245px] w-full py-2 md:px-4 px-2 rounded-md text-white font-bold ${
...
const PopupUse = ({ popupTranslations }) => {
  ...
  return (
    <div>
      <button
        onClick={openModal}
        className="h-[40px] md:w-fit w-full rounded-[5px] bg-white border border-black flex items-center justify-center text-black hover:border-opacity-10 md:mb-0 mb-3 transition-all"
      >
        <div className="text-[15px] font-bold px-5">
          {popupTranslations.REVIEWS_write_your_opinion?.toUpperCase() || 'WRITE YOUR REVIEW'}
        </div>
      </button>
      ...
      {isThankYouVisible && (
        <div
          className={`fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[203] transition-opacity ${
            isThankYouVisible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
          onClick={closeModal}
        >
          <div
          ...
          >
            ...
            <h5 className="text-sm font-bold h-[36px] w-[194px]">
              {popupTranslations.REVIEWS_modal_text || 'THANK YOU FOR YOUR FEEDBACK!'}
            </h5>
          </div>
        </div>
      )}
